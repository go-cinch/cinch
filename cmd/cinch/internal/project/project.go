...
var CmdNew = &cobra.Command{
	...
	Long:  "Create a service from git layout repository. Example: cinch new game",
	Run:   run,
}
...
func init() {
	repoURL = "https://github.com/go-cinch/layout.git"
	timeout = "60s"
	branch = "dev"
	CmdNew.Flags().StringVarP(&repoURL, "repo", "r", repoURL, "layout repo")
	CmdNew.Flags().StringVarP(&branch, "branch", "b", branch, "repo branch")
	CmdNew.Flags().StringVarP(&timeout, "timeout", "t", timeout, "execution timeout")
}
...
func run(_ *cobra.Command, args []string) {
	...
	if len(args) == 0 {
		prompt := &survey.Input{
			...
			Help:    "U custom project or service name. example: game",
		}
